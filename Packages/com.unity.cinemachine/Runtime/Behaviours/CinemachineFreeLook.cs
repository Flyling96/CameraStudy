...
namespace Cinemachine
{
    ...
    public class CinemachineFreeLook : CinemachineVirtualCameraBase
    {
        ...
        [AxisStateProperty]
        public AxisState m_YAxis = new AxisState(0, 1, false, false, 2f, 0.2f, 0.1f, "Mouse Y", false);

        [Tooltip("对Y轴进行动态限制")]
        [AxisLimitProperty]
        public AxisState.AxisLimit m_YAxisLimit = new AxisState.AxisLimit(null, null, 0.3f, new Vector3(0, 1, 0));
        ...
        [Tooltip("对X轴进行动态限制")]
        [AxisLimitProperty]
        public AxisState.AxisLimit m_XAxisLimit = new AxisState.AxisLimit(null,null, 30.0f, new Vector3(0, 0, 1));
        ...
        [Tooltip("在原有的弧线基础上加上额外半径调整")]
        public float m_ExtraRadius = 0;
        ...
        protected override void OnValidate()
        {
            ...
            m_YAxisLimit.Validate();
            m_XAxisLimit.Validate();
            m_Lens.Validate();
...
//            m_YAxis.m_Recentering.CancelRecentering();
        ...
        float UpdateXAxisHeading(CinemachineOrbitalTransposer orbital, float deltaTime, Vector3 up)
        {
            ...
            {
                m_XAxisLimit.DoAxisLimit(ref m_XAxis,new Vector3(0,1,0));
        ...
        //TODO:考虑向前发射射线，来平衡俯仰角权重，让俯仰角变化更自然，比如在悬崖时
        private float GetYRecenterTarget(Vector3 preCameraPos)
        {
            ...
            followDownPoint = followDownRay.GetPoint(hitInfo.distance);
            Vector3 dir = followDownPoint - cameraDownPoint;
            ...
            if (dir.y > 0)
            {
                    ...
                    QueryTriggerInteraction.Ignore))
                {
                    ...
                    Vector3 ffDir = forwardDownPoint - followDownPoint;
                    dir = ffDir.y < 0 ? ffDir : dir;
                }
            }
            ...
            float maxAbsY = dirXZ * Mathf.Tan(maxSlope);
            float middleAbsY = dirXZ * Mathf.Tan(middleSlope);

            recenterTarget -= dir.y > 0 ? dir.y / maxAbsY : (-dir.y > middleAbsY ? (dir.y + middleAbsY)/(maxAbsY - middleAbsY) : 0);
