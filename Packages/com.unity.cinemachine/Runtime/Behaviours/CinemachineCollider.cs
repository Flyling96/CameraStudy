...
namespace Cinemachine
{
    ...
    public class CinemachineCollider : CinemachineExtension
    {
        ...
        [Tooltip("限制相机距离目标的最近距离，为0的话就不进行限制")]
        public float m_MinDistanceNearTarget = 0.0f;
        ...
        public enum ResolutionStrategy
        {
            ...
            /// <summary>
            /// 在维持高度的基础上，维持相机与目标的最小距离，在与相机本身碰撞产生冲突时，采用维持现状卡住的方式
            /// </summary>
            PreserveCameraMinDistance,
            /// <summary>In addition to pulling the camera forward, an effort will be made to
        ...
        public ResolutionStrategy PreserveCameraMinDistance { get; private set; }
            ...
            CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
        {
            ...
            if (stage == CinemachineCore.Stage.Body)
            {
                if (m_AvoidObstacles)
                {
                    ...
                    Vector3 virtualCameraPos = state.RawPosition;

                    float damping = m_Damping;
                    Quaternion inverseRawOrientation = Quaternion.Inverse(state.RawOrientation);
                    //inverseRawOrientation = new Quaternion(inverseRawOrientation.x, 0, inverseRawOrientation.z, inverseRawOrientation.w);
                    displacement = inverseRawOrientation * displacement;
                    ...
                    displacement = Quaternion.Inverse(inverseRawOrientation) * displacement;
        ...
        private Vector3 PreserveLignOfSight(ref CameraState state, ref VcamExtraState extra)
        {
                ...
                && m_CollideAgainst != m_TransparentLayers)
            {
                ...
                displacement = PullCameraInFrontOfNearestObstacle(
                    state.PreviousPosition, cameraPos, lookAtPos, m_CollideAgainst & ~m_TransparentLayers, ref hitInfo);
        ...
        private Vector3 PullCameraInFrontOfNearestObstacle(
            Vector3 preCameraPos, Vector3 cameraPos, Vector3 lookAtPos, int layerMask, ref RaycastHit hitInfo)
        {
            Vector3 displacement = Vector3.zero;
            Vector3 dir = cameraPos - lookAtPos;
            Vector3 preDir = preCameraPos - lookAtPos;
            ...
            if (targetDistance > Epsilon)
            {
                ...
                else
                {
                    ...
                    rayLength = Mathf.Min(preDir.magnitude - minDistanceFromTarget + m_CameraRadius * 1.2f, rayLength);
